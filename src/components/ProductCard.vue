...
<template>
  <RouterLink :to="`/product/${id}`" class="block cursor-pointer group">
    <div class="bg-[#F0EEED] rounded-[20px] mb-4 overflow-hidden aspect-square flex items-center justify-center">
    ...
    <div class="flex items-center gap-1 my-1 md:my-2">
      <div class="flex text-[#FFC633] text-lg">
        <span v-for="(star, index) in getStarArray(rating)" :key="index">
            ...
            </template> <!-- Using a simple char, ideally SVG -->
    ...
    <div class="flex items-center gap-3">
        ...
        <span v-if="originalPrice" class="font-satoshi font-bold text-xl md:text-2xl text-black/40 line-through">${{ originalPrice }}</span>
        <span v-if="originalPrice" class="font-satoshi font-medium text-xs md:text-sm text-[#FF3333] bg-[#FF3333]/10 px-3 py-1 rounded-full">-{{ Math.round((1 - price/originalPrice) * 100) }}%</span>
    </div>
  </RouterLink>
</template>
