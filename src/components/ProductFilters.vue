...
<template>
  <div class="border border-black/10 rounded-[20px] p-5 bg-white">
    <div class="hidden lg:flex items-center justify-between mb-4 pb-4 border-b border-black/10">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" class="text-black/40"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
    </div>

    <!-- Categories List -->
    <ul class="space-y-3 mb-6 pb-6 border-b border-black/10 text-black/60 font-satoshi">
        <li v-for="cat in categories" :key="cat" class="flex justify-between items-center cursor-pointer hover:text-black">
            <span>{{ cat }}</span>
    ...
    <Accordion multiple :value="['0', '1', '2', '3']" class="filter-accordion">
        <!-- Price -->
         <AccordionPanel value="0">
            <AccordionHeader class="!bg-transparent !border-0 !px-0 font-bold text-xl font-integral">Price</AccordionHeader>
        ...
        <AccordionPanel value="1">
            <AccordionHeader class="!bg-transparent !border-0 !px-0 font-bold text-xl font-integral">Colors</AccordionHeader>
            <AccordionContent class="!px-0">
                <div class="flex flex-wrap gap-3">
                    ...
                    >
                         <svg v-if="selectedColor.includes(color.name)" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" :stroke="color.name === 'White' ? 'black' : 'white'" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
                    </button>
                </div>
            </AccordionContent>
        </AccordionPanel>
        ...
        <AccordionPanel value="2">
            <AccordionHeader class="!bg-transparent !border-0 !px-0 font-bold text-xl font-integral">Size</AccordionHeader>
            <AccordionContent class="!px-0">
        ...
        <AccordionPanel value="3">
            <AccordionHeader class="!bg-transparent !border-0 !px-0 font-bold text-xl font-integral">Dress Style</AccordionHeader>
             <AccordionContent class="!px-0">
                 <ul class="space-y-3 text-black/60 font-satoshi">
                    <li v-for="style in dressStyles" :key="style" class="flex justify-between items-center cursor-pointer hover:text-black">
                        <span>{{ style }}</span>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>
                    </li>
                </ul>
    ...
    <Button label="Apply Filter" class="!w-full !rounded-full !bg-black !text-white !font-satoshi !py-4 !mt-4 !border-0 hover:!bg-black/90" />
  </div>
</template>
